...
class ImageViewController: UIViewController, UIScrollViewDelegate {
    ...
    private func fetchImage() {
        if let url = imageURL {
            ...
            dispatch_async(dispatch_get_global_queue(qos, 0)) { () -> Void in
                let imageData = NSData(contentsOfURL: url) // this blocks the thread it isi ob
    ...
    // (will install it into the content area of the scroll ciew)
    override func viewDidLoad() {
        super.viewDidLoad()
        scrollView.addSubview(imageView)
        //        if image == nil {
        //            imageURL = DemoURL.TFboys
        //        }
    }
